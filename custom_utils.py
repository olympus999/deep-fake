...
def return_image_data_from_video(video_path, annotation, resize_width, frames):
    """
...
def return_image_data_from_video_wrapper(all_videos_batch, resize_width, frames):
    list_objects = []
        ...
        if len(list_objects) < videos_per_process * processes * len(frames):
            ...
            with Pool(processes=processes) as pool:
                    res = pool.starmap(return_image_data_from_video_wrapper, \
                                       zip(list_all_videos_batch, repeat(resize_width), repeat(frames)))
                    for r in res:
                        list_objects.extend(r)
        else:
            time.sleep(0.001)
            
def return_all_video_paths(train_path):
    ...
    for train_video_folder in train_video_folders:
        ...
        for key in metadata:
            ann = metadata[key]
            if ann['label'] == 'FAKE':
                fake_videos.append(join(train_video_folder, key))
                # original_videos.append(join(train_video_folder, ann['original']))
            else:
                original_videos.append(join(train_video_folder, key))

    original_videos = [[x, 0] for x in original_videos]
    fake_videos = [[x, 1] for x in fake_videos]

    all_videos = original_videos + fake_videos
    random.seed(4)
